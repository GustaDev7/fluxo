...
export async function loadFinanceData() {
  const [accountRows, cardRows, transactionRows, billRows, debtRows, investmentRows, goalRows, emergencyRows, preferenceRows] =
    await Promise.all([rows('finance_accounts'), rows('finance_cards'), rows('finance_transactions'), rows('finance_bills'), rows('finance_debts'), rows('finance_investments'), rows('financial_goals'), rows('emergency_funds'), rows('finance_preferences')]);
  ...
  return {
    ...
    debts: debtRows.map((r) => ({ id: r.id, creditor: r.creditor, type: r.debt_type, originalAmount: Number(r.original_amount), currentBalance: Number(r.current_balance), interestRateMonthly: Number(r.interest_rate_monthly), totalInstallments: r.total_installments, remainingInstallments: r.remaining_installments, installmentAmount: Number(r.installment_amount), dueDay: r.due_day, status: r.status, notes: r.notes, priority: 'medium' })),
...
export async function saveFinanceData(userId: string, data: any) {
  ...
  await replaceRows('finance_transactions', userId, data.transactions.map((r: any) => ({ id: r.id, user_id: userId, account_id: r.accountId || null, destination_account_id: r.destinationAccountId || null, card_id: r.cardId || null, project_id: r.projectId || null, linked_task_id: r.linkedTaskId || null, type: r.type, amount: r.amount, occurred_on: r.date, description: r.description, master_category: r.masterCategory || null, subcategory: r.subcategory || null, tags: r.tags || [], recurrence: { type: r.recurrence || 'none' }, notes: r.notes || null })));
