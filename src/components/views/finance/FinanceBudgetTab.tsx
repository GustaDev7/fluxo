...
const newIncome = (name = 'Salário'): BudgetIncomeSource => ({
  ...
  plannedAmount: 0, receivedAmount: 0, recurring: name === 'Salário',
});
...
export const FinanceBudgetTab: React.FC = () => {
  const { budget, selectedMonth, setSelectedMonth, updateBudgetPlan, transactions, monthlyClosingHistory, accounts, creditCards, openTransactionModal, addTransaction, updateTransaction, deleteTransaction } = useFinance();
  ...
  return <div className="space-y-5">
    <section>
      ...
      {result.plannedIncome === 0 && result.receivedIncome === 0 && <div className="mt-3 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-3 text-xs font-semibold text-indigo-700 dark:border-indigo-900 dark:bg-indigo-950/30 dark:text-indigo-300">Este mês está com a renda zerada. Informe apenas os valores previstos e recebidos desta competência.</div>}
    </section>
    ...
    {editing && <section className="rounded-3xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
      ...
      <div className="mt-4 space-y-2">{draft.incomeSources?.map((item) => <div key={item.id} className="relative grid grid-cols-2 gap-2 rounded-xl bg-neutral-50 p-3 sm:grid-cols-[minmax(0,1fr)_105px_105px_auto] sm:p-2 dark:bg-neutral-800/40">
        <input disabled={!editing} value={item.name} onChange={(e) => setDraft({ ...draft, incomeSources: draft.incomeSources?.map((s) => s.id === item.id ? { ...s, name: e.target.value } : s) })} className="col-span-2 min-w-0 bg-transparent px-2 pr-8 text-xs font-bold disabled:opacity-100 sm:col-span-1 sm:pr-2"/>
        <input disabled={!editing} aria-label="Previsto" title="Previsto" type="number" min="0" step="0.01" value={item.plannedAmount} onChange={(e) => setDraft({ ...draft, incomeSources: draft.incomeSources?.map((s) => s.id === item.id ? { ...s, plannedAmount: Number(e.target.value) } : s) })} className="min-w-0 rounded-lg border border-neutral-200 bg-white px-2 py-2 text-xs dark:border-neutral-700 dark:bg-neutral-900"/>
        <input disabled={!editing} aria-label="Recebido" title="Recebido" type="number" min="0" step="0.01" value={item.receivedAmount} onChange={(e) => setDraft({ ...draft, incomeSources: draft.incomeSources?.map((s) => s.id === item.id ? { ...s, receivedAmount: Number(e.target.value) } : s) })} className="min-w-0 rounded-lg border border-neutral-200 bg-white px-2 py-2 text-xs dark:border-neutral-700 dark:bg-neutral-900"/>
        {editing && <button type="button" aria-label="Excluir fonte" onClick={() => setDraft({ ...draft, incomeSources: draft.incomeSources?.filter((s) => s.id !== item.id) })} className="absolute right-2 top-2 rounded-lg p-1.5 sm:static sm:p-0"><Trash2 className="h-4 w-4 text-neutral-400"/></button>}
      </div>)}</div>
    </section>}
    ...
    <section>
      ...
      <div className={draft.viewMode === 'cards' ? 'grid gap-3 md:grid-cols-2 2xl:grid-cols-3' : 'space-y-2'}>{roots.map((category) => {
        const categoryTransactions = transactions.filter((tx) =>
          tx.type === 'expense'
          && tx.date.startsWith(draft.month)
          && transactionMatchesBudgetCategory(tx, category)
        );
        ...
        return <article key={category.id} className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          ...
          {editing ? <div className="mt-3 rounded-xl bg-neutral-50 p-3 dark:bg-neutral-800/50"><div className="grid grid-cols-2 gap-2"><button onClick={() => updateCategory(category.id, { allocationMode: 'percentage' })} className={`rounded-lg border px-3 py-2 text-xs font-bold ${category.allocationMode === 'percentage' ? 'border-indigo-500 bg-indigo-500/10 text-indigo-500' : 'border-neutral-200 dark:border-neutral-700'}`}>% da renda</button><button onClick={() => updateCategory(category.id, { allocationMode: 'fixed' })} className={`rounded-lg border px-3 py-2 text-xs font-bold ${category.allocationMode === 'fixed' ? 'border-indigo-500 bg-indigo-500/10 text-indigo-500' : 'border-neutral-200 dark:border-neutral-700'}`}>Valor fixo</button></div>{category.allocationMode === 'percentage' ? <label className="mt-3 block"><span className="text-[10px] font-bold uppercase tracking-wide text-neutral-500">Porcentagem da renda</span><div className="mt-1 flex items-center gap-2"><button onClick={() => updateCategory(category.id, { percentage: String(Math.max(0, Number(category.percentage) - 0.5)) })} className="h-9 w-9 rounded-lg border border-neutral-200 font-bold dark:border-neutral-700">−</button><div className="relative flex-1"><input aria-label="Porcentagem da renda" type="number" min="0" max="100" step="0.1" value={category.percentage} onChange={(e) => updateCategory(category.id, { percentage: e.target.value })} className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-2 pr-8 text-sm font-bold dark:border-neutral-700 dark:bg-neutral-900"/><span className="absolute right-3 top-2 text-sm text-neutral-400">%</span></div><button onClick={() => updateCategory(category.id, { percentage: String(Math.min(100, Number(category.percentage) + 0.5)) })} className="h-9 w-9 rounded-lg border border-neutral-200 font-bold dark:border-neutral-700">+</button></div><span className="mt-1 block text-[11px] text-neutral-500">Equivale a {formatBRL(category.plannedAmount)} neste mês.</span></label> : <label className="mt-3 block"><span className="text-[10px] font-bold uppercase tracking-wide text-neutral-500">Valor planejado</span><input aria-label="Valor planejado" type="number" min="0" step="0.01" value={category.fixedAmount} onChange={(e) => updateCategory(category.id, { fixedAmount: Number(e.target.value) })} className="mt-1 w-full rounded-lg border border-neutral-200 bg-white px-3 py-2 text-sm font-bold dark:border-neutral-700 dark:bg-neutral-900"/><span className="mt-1 block text-[11px] text-neutral-500">Representa {result.plannedIncome > 0 ? (category.plannedAmount / result.plannedIncome * 100).toFixed(1).replace('.', ',') : '0,0'}% da renda.</span></label>}</div> : <button onClick={() => setEditing(true)} className="mt-3 flex w-full items-center justify-between rounded-xl bg-neutral-50 px-3 py-2 text-left dark:bg-neutral-800/50"><span className="text-[11px] text-neutral-500">Forma de planejamento</span><b className="text-xs text-indigo-500">{category.allocationMode === 'percentage' ? `${Number(category.percentage).toFixed(1).replace('.', ',')}% da renda` : `${formatBRL(category.fixedAmount)} fixos`} · Editar</b></button>}
          ...
          <div className="mt-4 flex items-center justify-between border-t border-neutral-100 pt-3 dark:border-neutral-800"><span className="flex items-center gap-1.5 text-[11px] text-neutral-500"><ReceiptText className="h-3.5 w-3.5"/>{categoryTransactions.length} {categoryTransactions.length === 1 ? 'despesa' : 'despesas'}</span><div className="flex gap-2"><button onClick={() => openTransactionModal('expense', category.masterCategory || 'custos_fixos', { budgetCategoryId: category.masterCategory ? undefined : category.id, budgetCategoryName: category.masterCategory ? undefined : category.name, date: dateForBudgetMonth(draft.month) })} className="rounded-lg bg-indigo-600 px-2.5 py-1.5 text-[11px] font-bold text-white">+ Adicionar gasto</button><button onClick={() => setSelectedCategoryId(category.id)} className="rounded-lg border border-neutral-200 px-2.5 py-1.5 text-[11px] font-bold dark:border-neutral-700">Ver despesas</button></div></div>
        </article>;
      })}</div>
    ...
    {selectedCategoryId && <BudgetCategoryDetail
      ...
      onAdd={(category) => openTransactionModal('expense', category.masterCategory || 'custos_fixos', { budgetCategoryId: category.masterCategory ? undefined : category.id, budgetCategoryName: category.masterCategory ? undefined : category.name, date: dateForBudgetMonth(draft.month) })}
...
const BudgetCategoryDetail = ({ category, month, transactions, accounts, creditCards, onClose, onAdd, onDuplicate, onUpdate, onDelete }: {
  ...
  onClose: () => void; onAdd: (category: BudgetCategory) => void; onDuplicate: (transaction: Omit<FinanceTransaction, 'id'>) => void;
  ...
  const categoryTransactions = transactions.filter((tx) => tx.type === 'expense' && tx.date.startsWith(month) && transactionMatchesBudgetCategory(tx, category));
  const rows = categoryTransactions.filter((tx) => [tx.description, tx.subcategory, String(tx.amount)].some((value) => value?.toLowerCase().includes(search.toLowerCase()))).sort((a, b) => sort === 'highest' ? b.amount - a.amount : sort === 'lowest' ? a.amount - b.amount : b.date.localeCompare(a.date));
  const spent = rows.reduce((sum, tx) => sum + tx.amount, 0);
  const allSpent = categoryTransactions.reduce((sum, tx) => sum + tx.amount, 0);
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm"><section className="flex h-full w-full max-w-2xl flex-col bg-neutral-50 shadow-2xl dark:bg-neutral-950">
    <header className="border-b border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900 sm:p-6"><div className="flex items-start justify-between gap-4"><div className="flex items-center gap-3"><span className="h-4 w-4 rounded-full" style={{ backgroundColor: category.color }}/><div><h2 className="text-xl font-black">{category.name}</h2><p className="text-xs text-neutral-500">Detalhamento de {new Date(`${month}-01T00:00:00Z`).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric', timeZone: 'UTC' })}</p></div></div><button aria-label="Fechar" onClick={onClose} className="rounded-xl p-2 hover:bg-neutral-100 dark:hover:bg-neutral-800"><X className="h-5 w-5"/></button></div>
      <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4">{[['Orçamento', category.plannedAmount], ['Gasto', allSpent], [available >= 0 ? 'Disponível' : 'Excedente', Math.abs(available)], ['Lançamentos', categoryTransactions.length]].map(([label, value]) => <div key={String(label)} className="rounded-xl bg-neutral-100 p-3 dark:bg-neutral-800"><span className="text-[10px] uppercase tracking-wide text-neutral-500">{label}</span><b className={`mt-1 block text-sm ${label === 'Excedente' ? 'text-rose-500' : ''}`}>{label === 'Lançamentos' ? value : formatBRL(Number(value))}</b></div>)}</div>
    </header>
    ...
    <footer className="border-t border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900"><button onClick={() => onAdd(category)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 py-3 text-sm font-bold text-white"><Plus className="h-4 w-4"/>Adicionar gasto em {category.name}</button></footer>
  </section></div>;
};
